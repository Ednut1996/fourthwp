...
class WPF_Form {
    ...
    private function add_fields($data = array()) {
        ...
        <div class="wpf_lightbox_row">
            ...
            <div class="wpf_lightbox_input">
                <input 
                    <?php if ((!isset($data['group']) && empty($data)) || !empty($data['group'])): ?>checked="checked"<?php endif; ?>
    ...
    public function save_themplate(array $post) {
        ...
        if (!empty($post['layout'])) {
            $option = WPF_Options::get_option($this->plugin_name, $this->version);
            if (empty($post['name']) && empty($post['themplate_id'])) {
            ...
            $_keys = array('name', 'empty', 'group', 'type', 'page', 'sort', 'pagination', 'posts_per_page', 'result', 'scroll', 'result_type', 'tax_relation', 'pagination_type');
            foreach ($_keys as $k) {
                if (!empty($post[$k])) {
        ...
        switch ($type):
            case 'price':
                ...
                if (!empty($module['from'])) {
                    $k = key($module['from']);
                    $from = floatval($module['from'][$k]);
                    unset($module['from'][$k]);
                    $k = key($module['to']);
                    $to = floatval($module['to'][$k]);
                    unset($module['to'][$k]);
                }
                wp_reset_postdata();
                ?>
                        ...
                        <ul>
                            ...
                            <?php if (!empty($module['from'])): ?>
                                <?php foreach ($module['from'] as $i => $v): ?>
                                    <li>
                                        <span><?php _e('Price from', 'wpf') ?></span>
                                        <input min="0" max="<?php echo $max ?>" value="<?php echo floatval($v) ?>" name="[<?php echo $type ?>][from][]" type="number" />
                                        <span><?php _e('to', 'wpf') ?></span>
                                        <input min="<?php echo $min ?>" max="<?php echo $max ?>" value="<?php echo!empty($module['to'][$i]) ? floatval($module['to'][$i]) : '' ?>" name="[<?php echo $type ?>][to][]" type="number" />
                                        <span class="wpf_module_btn wpf_remove_item"></span>
                                    </li>
                                <?php endforeach; ?>
                            <?php endif; ?>
    ...
    protected function get_public_fields($type, array $args, array $data, array $request = array(), $lang = false) {
        ...
        switch ($type):
            ...
            case 'price':
                ...
                if ($price_type === 'slider') {
                    ...
                    if ($max_query->have_posts()) {
                        ...
                        <div data-max="<?php echo $max ?>" data-min="<?php echo $min ?>"  class="wpf_slider"></div>
                    ...
                    $selected = isset($value['from']) && is_numeric($value['from']) ? floatval($value['from']) : '';
                    $selected.='-';
                    $selected.= isset($value['to']) && is_numeric($value['to']) ? floatval($value['to']) : '';
                    if ($selected === '-') {
                        $selected = false;
                    }
                    ?>
                    <ul class="wpf_price_range">
                        ...
                        <?php foreach ($args['from'] as $i => $v): ?>
                            <?php
                            $from = floatval($v);
                            $to = !empty($args['to'][$i]) ? floatval($args['to'][$i]) : '';
                            $orig_v = $from . '-' . $to;
                            ?>
                            <li>
            ...
            default:
                ...
                unset($categories);
                if (empty($value) && empty($_REQUEST['wpf']) && (($type === 'wpf_cat' && is_product_category()) || ($type === 'wpf_tag' && is_product_tag()))) {
                    global $wp_query;
                    $cat = $wp_query->get_queried_object();
                    if (is_object($cat) && !empty($cat->slug)) {
                        $value = array($cat->slug);
                    }
                }
                ...
                <?php if (!empty($cats)): ?>
                    <?php if ($is_dropdown): ?>
                        <select name="<?php echo WPF_Utils::strtolower(WPF_Utils::get_field_name($args, $type)); ?><?php if ('multiselect' === $args['show_as']): ?>[]<?php endif; ?>" <?php if ('multiselect' === $args['show_as']): ?>multiple="multiple"<?php endif; ?>class="wpf_dropdown">
                        <?php else: ?>
                            ...
                            <?php endif; ?>
                            <?php echo $is_dropdown ? '</select>' : '</ul>'; ?>
                        <?php endif; ?>
                        <?php break; ?>
                <?php endswitch; ?>
                <?php
            ...
            private function category_walker($items, $cats = array(), $type, array $args = array(), $value, $hide_empty = true, $lang, $i = 1) {
                ...
                <?php foreach ($items as $cat): ?>
                    ...
                    <?php if ('dropdown' === $args['show_as'] || 'multiselect' === $args['show_as']): ?>
                        <option <?php if (in_array($cat->slug, $value)): ?>selected="selected"<?php endif; ?> value="<?php echo $cat->slug ?>">
                            <?php
                            if ($hierarchy && $i > 2) {
                    ...
                    <?php else: ?>
                        <li>
                            <?php if ('link' === $args['show_as']): ?>
                                <?php if (strpos($type, 'pa_') !== 0 && in_array($cat->slug, $value)): ?>
                                    <span class="wpf_selected"><?php echo $cat->name ?></span>
                                <?php else: ?>
                                    <?php if (strpos($type, 'pa_') === 0): ?>
                                        <a class="wpf_pa_link" href="<?php echo get_category_link($cat->term_id) ?>">
                                            <input <?php if (in_array($cat->slug, $value)): ?>checked="checked"<?php endif; ?> type="radio" value="<?php echo $cat->slug ?>" name="<?php echo $name ?>" />
                            ...
                            <?php else: ?>
                                ...
                                ?>
                                <input <?php if (in_array($cat->slug, $value)): ?>checked="checked"<?php endif; ?> id="wpf_<?php echo $this->themplate_id ?>_<?php echo $cat->term_id ?>" type="<?php echo $args['show_as'] ?>" name="<?php echo $name; ?>[]" value="<?php echo $cat->slug ?>" />
                                <label <?php if ($color && !empty($args['color_bg_' . $cat->term_id])): ?> style="background-color:<?php echo $args['color_bg_' . $cat->term_id] ?>;<?php if (!empty($args['color_text_' . $cat->term_id])): ?> color:<?php echo $args['color_text_' . $cat->term_id] ?>;<?php endif; ?>"<?php endif; ?> for="wpf_<?php echo $this->themplate_id ?>_<?php echo $cat->term_id ?>">
                                ...
                                <ul class="wpf_tax_items">
                                    <?php foreach ($categories as $cat): ?>
                                        <li>
                                            ...
                                            <label class="wpf_color_text" for="wpf_<?php echo $type ?>_text_<?php echo $cat->term_id ?>"><?php _e('Optional icon text', 'wpf') ?></label>
                                        </li>
                                    <?php endforeach; ?>
                                </ul>
