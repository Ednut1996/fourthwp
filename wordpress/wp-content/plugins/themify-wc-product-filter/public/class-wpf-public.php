...
class WPF_Public {
     ...
     * @return string $text
     */
    function widget_text($text, $instance) {
    ...
    public function result_container($content = '') {
        ...
        if (!empty($forms[$slug])) {
            ...
            $show_result_in_same_page = (!empty($template['data']['result_type']) && $template['data']['result_type'] === 'same_page' ) ? true : false;
            $show_form_in_results = ( empty($template['data']['show_form_in_results']) || $template['data']['show_form_in_results'] != 'show_form_in_results' ) ? false : true;
    ...
    public function hide_templates($located, $template_name, $args, $template_path, $default_path) {
        ...
        return in_array($template_name, $this->templates) ? $this->filter_not_found($located, 'loop/no-products-found.php', $args, $template_path, $default_path) : $located;
    }
    ...
    public function get_result(array $data, array $form) {
        ...
        if (!empty($query_args)) {
            ...
            if (!WPF_Utils::is_ajax() || (isset($_POST['wpf_page_id']) && wc_get_page_id('shop') != $_POST['wpf_page_id'])) {
                add_filter('woocommerce_show_page_title', array($this, 'hide_page_title'), 99, 1);
            } else {
            ...
            remove_action('woocommerce_after_shop_loop', array($this, 'wrap_pagination'), 1);
            if (!WPF_Utils::is_ajax()) {
                remove_filter('woocommerce_show_page_title', array($this, 'hide_page_title'), 99, 1);
            } else {
            ...
            } elseif (!WPF_Utils::is_ajax() && is_woocommerce() && (empty($form['data']['result']) || empty($form['data']['sort']))) {
                ob_start();
        ...
        foreach ($layout as $type => $item) {
            if ($type !== 'submit') {
                ...
                if (!empty($post[$key]) || (isset($post[$key]) && $post[$key] === '0')) {
                    ...
                    if ($build) {
                        $this->build_query($type, $val, $query_args, $item['logic']);
                    } else {
                        $args[$type] = $val;
                    }
                }
    ...
    public function build_query($type, $value, &$query_args, $logic = false) {
        switch ($type) {
            case 'title':
    ...
    public function hide_page_title($hide) {
        return false;
    }
