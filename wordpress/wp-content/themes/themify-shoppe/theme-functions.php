...
function themify_theme_enqueue_scripts() {
	...
	if( themify_is_woocommerce_active() ) {
		...
		if(themify_get_gallery_type()==='default'){
			if(version_compare(WOOCOMMERCE_VERSION,'3.0.0','>=' ) || get_option( 'woocommerce_enable_lightbox' ) === 'yes'){
		...
		wp_localize_script( 'theme-shop', 'themifyShop', apply_filters(
				...
				array(
					...
					'redirect'=> get_option( 'woocommerce_cart_redirect_after_add' ) === 'yes'?WC()->cart->get_cart_url():false,
					'theme_url'=>THEME_URI,
...
function themify_theme_fixed_header() {
	...
	if(is_null($header_fixed)){
		...
		if ( in_array( $header, array( 'none', 'header-boxed-content' ) ) ) {
...
if ( ! function_exists( 'themify_theme_do_not_exclude_all' ) ) {
	...
	function themify_theme_do_not_exclude_all( $area = 'header' ) {
		if ( 'mobile-menu' == $area ) {
			return themify_theme_show_area( 'search_icon' ) || themify_theme_show_area( 'menu_navigation' );
		} elseif ( 'header' == $area ) {
			return themify_theme_show_area( 'site_logo' ) || themify_theme_show_area( 'site_tagline' ) || themify_theme_show_area( 'search_icon' )  || themify_theme_show_area( 'menu_navigation' );
		} elseif ( 'footer' == $area ) {
...
function themify_theme_query_classes() {
	...
	if(!isset($themify->builder_args['is_product']) || $is_product_query_page){
		...
		$queried_object = get_queried_object();
		$is_product_query = themify_is_product_search() || (is_object($queried_object)?(is_array($queried_object->name)?in_array('product',$queried_object->name):$queried_object->name==='product'):false);
...
function themify_builder_args($show, $mod, $builder_id, $identifier) {
    ...
    if (isset($mod['mod_name'])) {
		if($mod['mod_name'] === 'products' || $mod['mod_name'] === 'post'){
			
			$is_product = $mod['mod_name'] === 'products';
			$layout = $is_product ? $mod['mod_settings']['layout_products'] : $mod['mod_settings']['layout_post'];
                        $masonary = in_array($layout, array('grid2', 'grid3', 'grid4')) && !empty($mod['mod_settings']['masonry_post']) ? $mod['mod_settings']['masonry_post'] : false;
			$content_layout = $layout!='auto_tiles' && !empty($mod['mod_settings']['content_layout']) ? $mod['mod_settings']['content_layout'] : false;
			$post_gutter = !empty($mod['mod_settings']['post_gutter']) ? $mod['mod_settings']['post_gutter'] : false;
			if ($is_product) {
				$masonary = $masonary === 'default' ? ('yes' == themify_get('setting-post_masonry')) : ($masonary === 'yes');
				$content_layout = $content_layout === 'default' ? themify_get('setting-product_content_layout') : ($content_layout !== 'no' ? $content_layout : false);
				$post_gutter = $post_gutter === 'default' ? themify_get('setting-post_gutter') === 'no-gutter' : ($post_gutter === 'no');
				$product_hover = !empty($mod['mod_settings']['disable_product_slider']) ? $mod['mod_settings']['disable_product_slider'] : false;
				$product_hover = $product_hover === 'default' ? themify_get('setting-products_slider') === 'enable' : ($product_hover === 'enable');
				$themify->builder_args['slider_hover'] = $product_hover ? 'slider_hover' : 'slider_no_hover';
				$themify->builder_args['is_product'] = true;
			} else {
				$masonary = $masonary === 'default' ? !themify_check('setting-shop_masonry_disabled') : ($masonary === 'yes');
				$content_layout = $content_layout === 'default' ? themify_get('setting-post_content_layout') : ($content_layout !== 'no' ? $content_layout : false);
				$post_gutter = $post_gutter === 'default' ? themify_get('setting-product_post_gutter') === 'no-gutter' : ($post_gutter === 'no');
			}
			if ($masonary) {
			...
			if ($post_gutter) {
				$themify->builder_args['gutter'] = 'no-gutter';
			}
			if ($is_product) {
				add_filter('themify_theme_loops_wrapper_class', 'themify_add_builder_classes', 10, 1);
			} elseif (!empty($themify->builder_args)) {
				add_filter('themify_builder_module_loops_wrapper', 'themify_add_builder_classes', 10, 1);
			}
		}
		elseif($mod['mod_name'] === 'fancy-heading' && isset( $mod['mod_settings']['inline_text'] ) && $mod['mod_settings']['inline_text']==1){
...
function themify_theme_bonus_addons_update( $match, $subs ) {
	...
	foreach ( $subs as $key => $value ) {
		if( stripos( $value['title'], $theme_name ) !== false && isset( $_POST['nicename_short'] ) && in_array( $_POST['nicename_short'], array( 'Slider Pro', 'Pricing Table', 'Maps Pro', 'Typewriter', 'Image Pro', 'Timeline', 'WooCommmerce', 'Contact', 'Counter', 'Progress Bar', 'Countdown', 'Audio' ) ) ) {
			$match = 'true';
			break;
		}
	}
